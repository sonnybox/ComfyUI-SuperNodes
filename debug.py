...
def _latent_stats_string(x: torch.Tensor, name: str = "latent") -> str:
    ...
    with torch.no_grad():
        ...
        # per-channel mean/std (averaged over batch)
        ch_mean = x.mean(dim=(0, 2, 3))
        ch_std = x.std(dim=(0, 2, 3), unbiased=False)

        # percentile snapshot (global, per batch item averaged)
        # Compute on a downsample to keep it fast if huge
        # (Still GPU-friendly)
        flat = x.reshape(B, -1)
        # sample up to ~1M values per batch item to keep quantile stable+fast
        if flat.shape[1] > 1_000_000:
            idx = torch.randint(0, flat.shape[1], (B, 1_000_000), device=x.device)
            flat_s = flat.gather(1, idx)
        else:
            flat_s = flat

        q = torch.quantile(flat_s, torch.tensor([0.001, 0.01, 0.5, 0.99, 0.999], device=x.device, dtype=x.dtype), dim=1)
        q = q.mean(dim=0)  # average across batch items
        ...
        s = (
            ...
            f"quantiles (avg over batch): q0.1%={_fmt(q[0].item())} q1%={_fmt(q[1].item())} "
...
class SuperLatentDeltaStats:
    ...
    def run(self, latent_a, latent_b, label="delta"):
        ...
        # Ensure same shape
        if a.shape != b.shape:
            return (f"{label}: shapes differ: {tuple(a.shape)} vs {tuple(b.shape)}",)
        ...
        with torch.no_grad():
            ...
            # cosine similarity as “direction” check
            af = a.flatten()
            bf = b.flatten()
